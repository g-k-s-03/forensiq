                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 w-28">
                      <span
                        className={`text-xs px-1.5 py-0.5 rounded font-medium ${
                          CONF_BADGE[ev.confidence] ?? 'bg-slate-700 text-slate-300'
                        }`}
                      >
                        {ev.confidence}
                      </span>
                    ...
                    <td className="px-3 py-2.5 text-xs text-slate-300 max-w-xs">
                      <span className="line-clamp-2" title={ev.description}>
                        {ev.description}
                      </span>
                    </td>
                  </tr>
...
function VisualView({ events }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* Three-column grid: timestamp | dot+line | card */}
      <div
        className="grid"
        style={{ gridTemplateColumns: '7rem 1.5rem 1fr' }}
      >
        {capped.map((ev, i) => {
          ...
          return (
            <Fragment key={i}>
              ...
              <div className="flex flex-col items-center">
                <div
                  className={`w-2.5 h-2.5 rounded-full ring-2 ring-slate-900 mt-2.5 flex-shrink-0 ${
                    CONF_DOT[ev.confidence] ?? 'bg-slate-500'
                  }`}
                />
                {!isLast && (
              ...
              <div className="pl-3 pb-3 min-w-0">
                <div
                  className={`border-l-2 ${
                    CONF_BORDER[ev.confidence] ?? 'border-slate-600'
                  } rounded-r-lg bg-slate-800/60 px-3 py-2.5`}
                >
...
export default function TimelineView({ timeline = [] }) {
  ...
  const [mode, setMode]           = useState('table') // 'table' | 'visual'
