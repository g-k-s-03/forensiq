...
export default function CookiesTable({ cookies = [] }) {
  if (cookies.length === 0) {
    return (
      <p className="text-center text-slate-500 text-xs py-8">No cookies found.</p>
    )
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-700 sticky top-0 z-10">
              {['#', 'Source', 'Host', 'Cookie Name', 'Expires', 'Secure', 'HttpOnly'].map(
                (h) => (
                  <th
                    key={h}
            ...
            {cookies.map((c, i) => (
              ...
              >
                ...
                <td className="px-3 py-2.5 w-28">
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded font-medium ${
                      SRC_BADGE[c.source] ?? 'bg-slate-700 text-slate-300'
                    }`}
                  >
                    {c.source}
                  </span>
                ...
                <td className="px-3 py-2.5 text-center w-16">
                  <BoolCell value={c.is_secure} />
                </td>

                <td className="px-3 py-2.5 text-center w-20">
                  <BoolCell value={c.is_httponly} />
                </td>
              </tr>
            ))}
          </tbody>
